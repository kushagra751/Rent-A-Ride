...
const syncLocalVendorApproval = async () => {
...
export const razorpayOrder = async (req, res, next) => {
  try {
    ...
    if (!razorpayKeyId || !razorpaySecret) {
      return next(errorHandler(503, "Razorpay is not configured on the server"));
    }
    ...
    const razorpayMessage =
      ...
      error?.error?.reason ||
      error?.message ||
      "error occured in razorpayorder";
    next(errorHandler(500, razorpayMessage));
  }
};
