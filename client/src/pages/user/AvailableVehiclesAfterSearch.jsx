...
const AvailableVehiclesAfterSearch = () => {
  ...
  const showVarients = async (model) => {
    try {
      ...
      const res = await fetch(getApiUrl("/api/user/getVehiclesWithoutBooking"), {
        method: "POST",
        headers: {
    ...
    <div>
      ...
      {!availableCars ||
        (availableCars.length == 0 && (
          <CarNotFound
            compact
            title="No vehicles available for this route"
            message="Try a nearby pickup point, adjust your dates, or open the full catalog to continue your booking."
          />
        ))}
    </div>
  );
