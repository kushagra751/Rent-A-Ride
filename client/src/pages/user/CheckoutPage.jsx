...
async function sendBookingDetailsEmail(
  toEmail,
  bookingDetails,
  dispatch
) {
  try {
    const sendEamil = await fetch(getApiUrl("/api/user/sendBookingDetailsEamil"), {
      method: "POST",
      headers: {
    ...
    const response = await sendEamil.json().catch(() => ({}));

    if (!sendEamil.ok) {
      dispatch(setisPaymentDone(false));
      console.log(response?.message || "something went wrong while sending email");
      return;
    }
...
const CheckoutPage = () => {
  ...
  // If the user navigates back/refreshes mid-checkout, ensure we don't get stuck in loading state.
  useEffect(() => {
  ...
  const baseTotal = (Number(price) || 0) * (Number(Days) || 1);
  const totalPrice =
    baseTotal > 0 ? Math.max(0, baseTotal + 50 - (Number(discount) || 0)) : 0;
          ...
          >
            ...
            <div className="flex flex-col rounded-lg bg-white sm:flex-row">
              <img
                ...
                src={getVehicleImage(singleVehicleDetail)}
                alt={singleVehicleDetail?.model || singleVehicleDetail?.name || "Vehicle"}
              />
