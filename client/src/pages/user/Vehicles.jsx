...
import { setFilteredData, setVariantModeOrNot } from "../../redux/user/sortfilterSlice";
...
import { onVehicleDetail } from "./vehicleActions";
...
const Vehicles = () => {
  ...
  const visibleVehicles = Array.isArray(filterdData) ? filterdData : userAllVehicles;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(getApiUrl("/api/user/listAllVehicles"),{
        ...
        <div className=" flex  sm:flex-row  w-full  lg:grid lg:max-w-[1000px]  lg:grid-cols-3 justify-center items-center gap-5 flex-wrap mt-5">
          {Array.isArray(visibleVehicles) && visibleVehicles.length > 0 ? (
            ...
            <div className="max-w-[400px] flex flex-col justify-center gap-y-4 items-center mx-auto mt-10">
              <p className="text-md font-bold">No cars found for the selected filters</p>
            </div>
          )}
              </div>
              }
